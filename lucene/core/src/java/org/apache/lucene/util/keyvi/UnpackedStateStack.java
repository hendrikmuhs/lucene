...
/** Stack implementation for unpacked states with object pooling. */
public final class UnpackedStateStack {
  ...
  public void insertFinalState(int position, int transitionValue, boolean noMinimization) {
    ...
    // only do it explicit if no_minimization is true, it could be that the state is already marked,
    // so we should
    // not overwrite it.
    if (noMinimization) {
