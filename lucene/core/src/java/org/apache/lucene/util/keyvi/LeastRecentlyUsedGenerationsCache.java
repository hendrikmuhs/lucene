 ...
 *
 * <p>Used to de-duplicate/find equal states.
 */
public class LeastRecentlyUsedGenerationsCache<T extends MinimizationHashEntry> {
  ...
  public LeastRecentlyUsedGenerationsCache(T entryType, long memoryLimit) {
    ...
    MinimizationHash<T>.MemoryLimitConfiguration memoryConfiguration =
        currentGeneration.findMemoryLimitConfiguration(memoryLimit, 3, 6);
  ...
  public LeastRecentlyUsedGenerationsCache(
      T entryType, int sizePerGeneration, int maxNumberOfGenerations) {
    this.entryType = entryType;
