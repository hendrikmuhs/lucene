...
/**
 * A memory-efficient (multi-)set that can return an "equal" object (as opposed to "same"). This
 * consumes less memory than standard collections. When looking up a key, the key may be of a
 * different type, but it must be possible to map it to an entry of type T. It is a multi-set in
 * that it can contain more than one "equal" object, but it will only return the first one.
 */
public class MinimizationHash<T extends MinimizationHashEntry> {

  /** Helper class to calculate memory related configuration parameters given a fixed budget. */
  public class MemoryLimitConfiguration {
    ...
    public MemoryLimitConfiguration(
        long memoryLimit, int generations, int maxNumberOfItemsPerTable) {
  ...
  private static int[] HASH_SIZE_STEP_TABLE = {
    997,
    2029,
    4079,
    8171,
  ...
  MemoryLimitConfiguration findMemoryLimitConfiguration(long memoryLimit, int min, int max) {
    ...
    // try to find a good number of generations (between given minimum and maximum) with equal size
    // given the memory
    // Limit
    for (int i = min; i <= max; ++i) {
      ...
      for (int step = 3; step < HASH_SIZE_STEP_TABLE.length; ++step) {
        // the memory usage is the size of the hashtable itself and the size of the array for the
        // overflow buckets which
        // is the number of buckets divided by 4.
        long itemForecastForHashtable =
            HASH_SIZE_STEP_TABLE[step]
                + Math.min(HASH_SIZE_STEP_TABLE[step] >> 2, entry.getMaxCookieSize());
    ...
    return new MemoryLimitConfiguration(
        bestFitMemoryLimit, bestFitGenerations, bestFitMaximumNumberOfItemsPerTable);
  }
  ...
  */
  ...
  /**
   * Perform a hash lookup. If the hash values are equal, the key's equality operator is called upon
   * each entry with the same hash value.
   *
  ...
  public boolean get(Object key, T.Key entry) {
    ...
    while (entry.isEmpty() == false) {
      ...
      if (overflowBucket != 0) {
        entry.set(
            overflowEntriesOffsets[overflowBucket],
            overflowEntriesHashes[overflowBucket],
    ...
    if (entry.isEmpty() == false) {
      ...
      if (overflowBucket != 0) {
        ...
        if (key.equals(entry)) {
          ...
          entriesCookies[bucket] =
              entry.recalculateExtra(entriesCookies[bucket], entry.getCookie());
  ...
  public void add(T.Key key) {
    ...
    if (overflowCount == overflowEntriesSize
        && overflowEntriesSize < entry.getMaxCookieSize()
  ...
  private void growAndRehash(T.Key key) {
    ...
    for (int i = 1; i < oldOverflowCount; ++i) {
      key.set(
          oldOverflowEntriesOffsets[i], oldOverflowEntriesHashes[i], oldOverflowEntriesCookies[i]);
